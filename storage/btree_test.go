...
func TestBTreeInsertAndSearch(t *testing.T) {
	...
	// 插入几个 key-value
	if err := bt.Insert(10, []byte("ten")); err != nil {
		t.Fatalf("insert 10: %v", err)
	}
	if err := bt.Insert(5, []byte("five")); err != nil {
		t.Fatalf("insert 5: %v", err)
	}
	if err := bt.Insert(20, []byte("twenty")); err != nil {
		t.Fatalf("insert 20: %v", err)
	}

	// 查找
	val, found, err := bt.Search(10)
	if err != nil {
...
func TestBTreeInsertMany(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		...
		if err := bt.Insert(key, []byte(val)); err != nil {
...
func TestBTreeRangeScan(t *testing.T) {
	...
	// 范围扫描 [15, 35]
	results, err := bt.RangeScan(15, 35)
	if err != nil {
	...
	if results[0].Key != 20 || string(results[0].Value) != "v20" {
		t.Fatalf("result[0]: expected {20, v20}, got %v", results[0])
	}
	if results[1].Key != 30 || string(results[1].Value) != "v30" {
...
func TestBTreeUpdate(t *testing.T) {
	...
	// 插入
	if err := bt.Insert(1, []byte("old")); err != nil {
		t.Fatalf("insert: %v", err)
	}

	// 更新同一 key
	if err := bt.Insert(1, []byte("new")); err != nil {
		t.Fatalf("update: %v", err)
	}

	val, found, err := bt.Search(1)
	if err != nil {
